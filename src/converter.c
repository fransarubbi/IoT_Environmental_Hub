    ...
    while (1) {
        if (xQueueReceive(queues.flag, &flag, portMAX_DELAY) == pdTRUE) {
            ...
            switch (state) {
                case CHECK_FIRMWARE:
                    if (flag == 0) {
                        ESP_LOGI(TAG, "Estado: CHECK_FIRMWARE, Flag: 0");
                        Event event = eNotUpdate;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == UPDATE_FLAG) {
                        ESP_LOGI(TAG, "Estado: CHECK_FIRMWARE, Flag: UPDATE_FLAG");
                        Event event = eUpdate;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    break;
                ...
                case INIT_SYSTEM:
                    ...
                    if (flag == STATE_SAFE_MODE) {
                        ESP_LOGI(TAG, "Estado: INIT_SYSTEM, Flag: STATE_SAFE_MODE");
                        delete_timer(INIT_SYSTEM_TIMER);
                        Event event = eFromInitToSafe;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == STATE_BALANCE_MODE) {
                        ESP_LOGI(TAG, "Estado: INIT_SYSTEM, Flag: STATE_BALANCE_MODE");
                        delete_timer(INIT_SYSTEM_TIMER);
                        Event event = eFromInitToBalance;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == STATE_NORMAL) {
                        ESP_LOGI(TAG, "Estado: INIT_SYSTEM, Flag: STATE_NORMAL");
                        delete_timer(INIT_SYSTEM_TIMER);
                ...
                case UPDATE:
                    if (flag == UPDATE_OK) {
                        ESP_LOGI(TAG, "Estado: UPDATE, Flag: UPDATE_OK");
                        Event event = eUpdateOk;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == 0) {
                        ESP_LOGI(TAG, "Estado: UPDATE, Flag: 0");
                ...
                case INIT_BALANCE_MODE:
                    ...
                    if (flag == HANDSHAKE_REQUEST) {
                        ESP_LOGI(TAG, "Estado: INIT_BALANCE_MODE, Flag: HANDSHAKE_REQUEST");
                        Event event = eFromInitBalanceToInHandshake;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == PHASE_ALERT) {
                        ESP_LOGI(TAG, "Estado: INIT_BALANCE_MODE, Flag: PHASE_ALERT");
                        Event event = eFromInitBalanceToAlert;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == PHASE_DATA) {
                        ESP_LOGI(TAG, "Estado: INIT_BALANCE_MODE, Flag: PHASE_DATA");
                        Event event = eFromInitBalanceToData;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == PHASE_MONITOR) {
                        ESP_LOGI(TAG, "Estado: INIT_BALANCE_MODE, Flag: PHASE_MONITOR");
                        Event event = eFromInitBalanceToMonitor;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == STATE_SAFE_MODE) {
                        ESP_LOGI(TAG, "Estado: INIT_BALANCE_MODE, Flag: STATE_SAFE_MODE");
                ...
                case IN_HANDSHAKE:
                    ...
                    if (flag == PHASE_ALERT) {
                        ESP_LOGI(TAG, "Estado: IN_HANDSHAKE, Flag: PHASE_ALERT");
                        Event event = eFromInHandshakeToAlert;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == NEWER_EPOCH) {
                        ESP_LOGI(TAG, "Estado: IN_HANDSHAKE, Flag: NEWER_EPOCH");
                        Event event = eNewerEpoch;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == STATE_SAFE_MODE) {
                        ESP_LOGI(TAG, "Estado: IN_HANDSHAKE, Flag: STATE_SAFE_MODE");
                ...
                case ALERT:
                    ...
                    if (flag == PHASE_DATA) {
                        ESP_LOGI(TAG, "Estado: ALERT, Flag: PHASE_DATA");
                        Event event = eFromAlertToData;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == NEWER_EPOCH) {
                        ESP_LOGI(TAG, "Estado: ALERT, Flag: NEWER_EPOCH");
                        Event event = eNewerEpoch;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    break;
                ...
                case DATA:
                    if (flag == TIMEOUT_HEARTBEAT) {
                        ESP_LOGI(TAG, "Estado: DATA, Flag: TIMEOUT_HEARTBEAT");
                        Event event = eFromDataToStore;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == PHASE_MONITOR) {
                        ESP_LOGI(TAG, "Estado: DATA, Flag: PHASE_MONITOR");
                        Event event = eFromDataToMonitor;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == NEWER_EPOCH) {
                        ESP_LOGI(TAG, "Estado: DATA, Flag: NEWER_EPOCH");
                        Event event = eNewerEpoch;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                ...
                case MONITOR:
                    ...
                    if (flag == NEWER_EPOCH) {
                        ESP_LOGI(TAG, "Estado: MONITOR, Flag: NEWER_EPOCH");
                        Event event = eNewerEpoch;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                ...
                case OUT_HANDSHAKE:
                    ...
                    if (flag == STATE_NORMAL) {
                        ESP_LOGI(TAG, "Estado: OUT_HANDSHAKE, Flag: NORMAL");
                        Event event = eFromOutHandshakeToNormal;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == NEWER_EPOCH) {
                        ESP_LOGI(TAG, "Estado: OUT_HANDSHAKE, Flag: NEWER_EPOCH");
                        Event event = eNewerEpoch;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == STATE_SAFE_MODE) {
                        ESP_LOGI(TAG, "Estado: OUT_HANDSHAKE, Flag: STATE_SAFE_MODE");
                ...
                case NORMAL:
                    if (flag == STATE_BALANCE_MODE) {
                        ESP_LOGI(TAG, "Estado: NORMAL, Flag: STATE_BALANCE_MODE");
                ...
                case COOLING_TIME:
                    if (flag == TIMEOUT_COOLING) {
                        ESP_LOGI(TAG, "Estado: COOLING_TIME, Flag: TIMEOUT_COOLING");
                ...
                case UPDATE_SCORE:
                    if (flag == HEALTH_SCORE_DEGRADED) {
                        ESP_LOGI(TAG, "Estado: UPDATE_SCORE, Flag: HEALTH_SCORE_DEGRADED");
                        Event event = eFromUpdateScoreToNormal;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == HEALTH_SCORE_UNAVAILABLE || flag == HEALTH_SCORE_CRITICAL) {
                        ESP_LOGI(TAG, "Estado: UPDATE_SCORE, Flag: HEALTH_SCORE_UNAVAILABLE o HEALTH_SCORE_CRITICAL");
                ...
                case BYPASS:
                    if (flag == STATE_BALANCE_MODE) {
                        ESP_LOGI(TAG, "Estado: BYPASS, Flag: STATE_BALANCE_MODE");
                        Event event = eFromBypassToBalance;
                        xQueueSend(queues.event, &event, pdMS_TO_TICKS(100));
                    }
                    if (flag == TIMEOUT_BYPASS) {
                        ESP_LOGI(TAG, "Estado: BYPASS, Flag: TIMEOUT_BYPASS");
                ...
                case SAFE_MODE:
                    ...
                    if (flag == TIMEOUT_HEARTBEAT) {
                        ESP_LOGI(TAG, "Estado: SAFE_MODE, Flag: TIMEOUT_HEARTBEAT");
